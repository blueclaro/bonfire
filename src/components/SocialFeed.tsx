...
export default function SocialFeed() {
  ...
  return <section aria-label="Feed da comunidade" className="mx-auto max-w-2xl">
    ...
    <div className="space-y-4">{entries.map(entry=><article key={entry.event_id} className="rounded-2xl border border-white/10 bg-white/[.035] p-4 sm:p-5">
      {entry.igniter && <p className="mb-3 text-xs text-[#ffd19a]">🔥 <Link href={`/pessoas/${entry.igniter.id}`}>{socialHandle(entry.igniter)}</Link> deu um Ignite</p>}
      ...
      <div className="mt-4 flex flex-wrap gap-2 border-t border-white/10 pt-3 text-sm"><button aria-pressed={entry.liked} disabled={!!busy} onClick={()=>void react(entry,'like')} className={`min-h-11 rounded-full px-3 ${entry.liked?'bg-[#ff8a3d]/15 text-[#ffd19a]':'bg-white/5'}`}>♥ {entry.likes} Curtidas</button><Link href={`/foruns/topico/${entry.id}#novo-comentario`} className="inline-flex min-h-11 items-center rounded-full bg-white/5 px-3">↩ {entry.comments} Comentários</Link><button aria-pressed={entry.ignited} disabled={!!busy || !entry.can_ignite} title={!entry.can_ignite?'Conteúdo restrito não pode ser repostado':undefined} onClick={()=>void react(entry,'ignite')} className={`min-h-11 rounded-full px-3 disabled:opacity-50 ${entry.ignited?'bg-[#ff8a3d]/15 text-[#ffd19a]':'bg-white/5'}`}>🔥 {entry.ignites} Ignites</button></div>
