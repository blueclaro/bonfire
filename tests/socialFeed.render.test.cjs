...
test('feed autenticado oferece compositor, perfil, comentários e reações',()=>{
  ...
  for(const expected of ['PPO 2026','O que está acontecendo?','Publicar','type="file"','@Charlie#eba','curtidas','comentários','Ignites','aria-pressed="true"','/pessoas/charlie'])assert.ok(html.includes(expected),expected);
