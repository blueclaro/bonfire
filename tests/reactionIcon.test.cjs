...
test('reação toca uma vez após carregar e retorna ao frame estático',()=>{
  const h=harness();let icon=h.render({kind:'like',playKey:1});assert.equal(icon.props.src,'/like.gif?play=1');icon.props.onLoad();h.finish();assert.equal(h.render({kind:'like',playKey:1}).props.src,'/like-still.png');
